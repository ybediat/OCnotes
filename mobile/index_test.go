...
// La vue globale passe par l'index plutôt que par ListFolderJSON. Elle doit
// pourtant transmettre le même statut de lecture seule, en ligne comme depuis
// l'inventaire hors connexion, sinon tous les formats reçoivent la même icône.
func TestListAllJSONMarqueLesDocumentsEnLectureSeule(t *testing.T) {
	app, serveur, _ := prepare(t)

	serveur.mu.Lock()
	serveur.files["Notes/rapport.docx"] = []byte("document")
	serveur.etags["Notes/rapport.docx"] = serveur.nextETag()
	serveur.files["Notes/carnet.odt"] = []byte("document")
	...
	verifie := func(t *testing.T, contexte string) {
		...
		raw, err := app.ListAllJSON()
		if err != nil {
			t.Fatalf("ListAllJSON %s: %v", contexte, err)
		}
		...
		for _, entree := range listing.Entries {
			attendu, concernee := attendus[entree.Name]
			if !concernee {
				continue
			}
			if entree.ReadOnly != attendu {
				t.Errorf(
					"%s: readOnly de %q = %v, attendu %v",
					contexte,
					entree.Name,
					entree.ReadOnly,
					attendu,
			...
			delete(attendus, entree.Name)
		}
		if len(attendus) != 0 {
			t.Errorf("%s: entrées absentes de l'inventaire: %v", contexte, attendus)
		}
	}
