...
func Odt(data []byte) ([]markdown.Block, error) {
	...
	a := &odt{
		...
		sautAvant: map[string]bool{},
		sautApres: map[string]bool{},
	}
	...
	return sansSautsSuperflus(a.out), nil
}
...
type odt struct {
	...
	// sautAvant / sautApres : style de paragraphe -> il force une nouvelle page
	// avant / après lui. L'ODF pose fo:break-before="page" dans les propriétés
	// de paragraphe du style, là où OOXML le met dans le paragraphe lui-même.
	sautAvant map[string]bool
	sautApres map[string]bool
...
func (a *odt) collecterStyles(data []byte) {
	...
	for {
		...
		case xml.StartElement:
			...
			case estS(t.Name, "paragraph-properties") && courant != "":
				if attr(t, nsFo, "break-before") == "page" {
					a.sautAvant[courant] = true
				}
				if attr(t, nsFo, "break-after") == "page" {
					a.sautApres[courant] = true
				}
...
func misesEnForme(e xml.StartElement) []markdown.Style {
	...
	if fondColore(attr(e, nsFo, "background-color")) {
		formes = append(formes, markdown.StyleHighlight)
	}
	return formes
}
...
func (a *odt) titre(d *xml.Decoder, ouverture xml.StartElement) error {
	...
	a.sautEventuel(a.sautAvant[style])
	var c constructeur
	...
	a.sautEventuel(a.sautApres[style])
	return nil
}

// sautEventuel pose un marqueur de saut de page quand le style du bloc en
// impose un. Les marqueurs en tête, en fin ou en double sont élagués plus tard
// par sansSautsSuperflus : rien à vérifier ici.
func (a *odt) sautEventuel(saut bool) {
	if saut {
		a.push(markdown.Block{Kind: markdown.KindPageBreak})
	}
}
...
func (a *odt) paragraphe(d *xml.Decoder, ouverture xml.StartElement, ctx contexteOdt) error {
	...
	if n := a.niveauDeStyle(style); n > 0 {
