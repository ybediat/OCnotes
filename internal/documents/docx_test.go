...
func TestDocxMiseEnForme(t *testing.T) {
	...
	cas := []struct {
		...
		// LibreOffice écrit un background-color HTML en w:shd sur le run, pas
		// en w:highlight : c'est l'autre voie que l'analyseur doit lire.
...
// TestDocxSautDePage fige le repère de saut de page explicite.
//
// Le HTML source porte un <p style="page-break-before: always"> : LibreOffice
// en fait un <w:br w:type="page"/> en tête du paragraphe, avant le moindre
// caractère. Le marqueur doit donc précéder ce paragraphe, pas le suivre.
func TestDocxSautDePage(t *testing.T) {
	verifieSautDePage(t, lireDocx(t, "exemple.docx"))
}
...
func blocContenant(t *testing.T, blocs []markdown.Block, extrait string) markdown.Block {
	t.Helper()
	return blocs[indiceBlocContenant(t, blocs, extrait)]
}

func indiceBlocContenant(t *testing.T, blocs []markdown.Block, extrait string) int {
	t.Helper()
	for i, b := range blocs {
...
// verifieSautDePage est commun aux deux formats : la fixture décrit le même
// document, et les deux analyseurs doivent poser le même repère au même endroit.
func verifieSautDePage(t *testing.T, blocs []markdown.Block) {
	t.Helper()

	if n := compte(blocs, markdown.KindPageBreak); n != 1 {
		t.Fatalf("%d sauts de page, attendu 1 (les doublons et les bords sont élagués)", n)
	}
	i := indiceBlocContenant(t, blocs, "apres le saut de page")
	if i == 0 || blocs[i-1].Kind != markdown.KindPageBreak {
		t.Errorf("le saut de page ne précède pas le paragraphe qui le suit : bloc %d = %s", i-1, bloc(blocs, i-1).Kind)
	}
	if blocs[i-1].Text != "" || len(blocs[i-1].Spans) != 0 {
		t.Errorf("le marqueur de saut de page porte du contenu : %+v", blocs[i-1])
	}
}
