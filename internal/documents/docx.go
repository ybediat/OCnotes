...
func (a *docx) paragraphe(d *xml.Decoder) error {
	...
	for {
		...
		case xml.StartElement:
			...
			case estW(t.Name, "pageBreakBefore"):
				// Posé dans w:pPr : le paragraphe commence une nouvelle page.
				sautAvant = actif(t)
...
func (a *docx) run(d *xml.Decoder, c *constructeur) error {
	debut := c.n
	var gras, italique, souligne, barre, surligne bool
	...
	for {
		...
		case xml.StartElement:
			...
			case estW(t.Name, "strike"):
				barre = actif(t)
			case estW(t.Name, "highlight"):
				// L'outil surligneur de Word : une couleur nommée, ou « none ».
				surligne = actif(t)
			case estW(t.Name, "shd"):
				// L'autre voie du surlignage : une trame de fond posée sur le
				// run. w:fill vaut « auto » ou une couleur hexadécimale ; c'est
				// ce que LibreOffice écrit pour un background-color HTML.
				if fondColore(attr(t, nsWord, "fill")) {
					surligne = true
				}
			case estW(t.Name, "t"):
			...
			case estW(t.Name, "tab"):
				c.write("\t")
			case estW(t.Name, "br"):
				// Un saut de page ou de colonne explicite se retient pour le
				// paragraphe, de son côté du texte ; les autres w:br sont de
				// simples retours à la ligne.
				switch attr(t, nsWord, "type") {
				case "page", "column":
					if c.n == 0 {
						c.sautAvant = true
					} else {
		...
		case xml.EndElement:
			if estW(t.Name, "r") {
				...
				if surligne {
					c.span(debut, markdown.StyleHighlight, "")
				}
				return nil
			}
		}
