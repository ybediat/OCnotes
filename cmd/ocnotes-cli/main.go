// Commande ocnotes-cli est un harnais de test desktop pour le cœur métier.
//
...
//
// L'App Token se transmet par la variable d'environnement OCNOTES_APP_TOKEN,
...
const usageText = `ocnotes-cli — harnais de test du client OpenCloud

Usage :
  ocnotes-cli [options] <commande> [arguments]
...
Commandes :
  ...
  cp <source> <cible>    copie une note vers un chemin complet
  rm <chemin>            supprime

Les options doivent précéder la commande.

Authentification :
  L'App Token se lit dans la variable d'environnement OCNOTES_APP_TOKEN.
  OCNOTES_SERVER et OCNOTES_USER fournissent les valeurs par défaut de
  -server et -user.

Exemple :
  $env:OCNOTES_APP_TOKEN = "..."
  ocnotes-cli -server https://cloud.exemple.fr -user moi ls Notes

Options :
`

func run(args []string) error {
	fs := flag.NewFlagSet("ocnotes-cli", flag.ContinueOnError)
	...
	server := fs.String("server", os.Getenv("OCNOTES_SERVER"), "URL du serveur OpenCloud")
	user := fs.String("user", os.Getenv("OCNOTES_USER"), "nom d'utilisateur, ou UUID du compte si l'IdP est en autoprovisioning")
	...
	token := os.Getenv("OCNOTES_APP_TOKEN")
	switch {
	case *server == "":
		return errors.New("serveur non indiqué : utiliser -server ou OCNOTES_SERVER")
	case *user == "":
		return errors.New("utilisateur non indiqué : utiliser -user ou OCNOTES_USER")
	case token == "":
		return errors.New("App Token absent : définir OCNOTES_APP_TOKEN")
	}
	...
	case "cp":
		return cmdCopy(ctx, space, cmdArgs)
	case "rm":
...
func openSpace(ctx context.Context, c *opencloud.Client, wanted string) (*opencloud.Space, error) {
	...
	if wanted != "" {
		...
		return nil, fmt.Errorf("espace %q introuvable (voir « ocnotes-cli drives »)", wanted)
	}
...
// cmdCopy est une copie brute vers un chemin complet : lecture puis écriture,
// sans suffixe « (2) » ni contrôle de nom. La façade mobile, elle, résout les
// collisions et refuse dossiers et documents — ce harnais reste au ras du
// transport, comme mv et put.
func cmdCopy(ctx context.Context, s *opencloud.Space, args []string) error {
	if len(args) != 2 {
		return errors.New("usage: cp <source> <cible>")
	}
	content, _, err := s.Read(ctx, args[0])
	if err != nil {
		return err
	}
	etag, err := s.Write(ctx, args[1], content, "")
	if err != nil {
		return err
	}
	fmt.Fprintf(os.Stderr, "%d octets copiés vers %s, ETag %s\n", len(content), args[1], etag)
	return nil
}
